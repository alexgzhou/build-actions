...
if [[ -n "$(ls -A "openwrt/config_bf" 2>/dev/null)" ]]; then
	...
	elif [[ -n "$(ls -A "openwrt/.Lede_core" 2>/dev/null)" ]]; then
		export firmware="Lede_source_XiaomiR4A"
		export CODE="lede"
		export Modelfile="Lede_source_XiaomiR4A"
		export Core=".Lede_core"
		source openwrt/.Lede_core
...
[[ ! ${YUAN_MA} == "false" ]] && {
	...
	echo
	TIME l " 2. Lede_5.4内核,LUCI 18.06版本(Lede_source_XiaomiR4A)"
	echo
	TIME l " 3. Lienol_4.14内核,LUCI 19.07版本(Lienol_source)"
	echo
	TIME l " 4. Immortalwrt_5.4内核,LUCI 21.02版本(Mortal_source)"
	echo
	TIME l " 5. N1和晶晨系列CPU盒子专用(openwrt_amlogic)"
	echo
	TIME l " 6. 退出编译程序"
	echo
	echo
	echo
	while :; do
	TIME g "请选择编译源码,输入[ 1、2、3、4、5、6 ]然后回车确认您的选择！"
	...
	case $CHOOSE in
		...
		2)
			...
			export Core=".Lede_core"
			export Modelfile="Lede_source_XiaomiR4A"
			source Lede_source_XiaomiR4A/.Lede_core > /dev/null 2>&1
			TIME y "您选择了：Lede_5.4内核,LUCI 18.06版本"
		break
		;;
		3)
...
elif [[ $firmware == "Lede_source_XiaomiR4A" ]]; then
	rm -rf openwrt && git clone https://github.com/coolsnowwolf/lede openwrt
	[[ $? -ne 0 ]] && {
		TIME r "源码下载失败，请检测网络或更换节点再尝试!"
		echo
...
echo
rm -rf ../{Lede_source,Lede_source_XiaomiR4A,Lienol_source,Mortal_source,openwrt_amlogic}
# 为编译做最后处理
